...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Engine | Implemented | `UIMd3Language` singleton: `UIMd3LanguageMode_{English,Cantonese,Bilingual}`, `text()`, `registerText()`. `src/VBox/Frontends/VirtualBox/src/md3/UIMd3Language.{h,cpp}` | None dedicated (mentioned in passing across several articles) | N/A (engine) | T1 — still source-pattern only, but since this audit `tools/md3/check-language-registry.ps1` (LocalGates gate 19) derives its expectations from the source instead of a hand-written pattern list and checks all 427 statically visible `registerText` registrations for an empty key, empty English, empty Cantonese, and conflicting duplicate keys. It found and forced the fix of one real conflict (`md3.wizard.current-page-description`). It does **not** check the 40 registrations behind the two runtime-parameterised call sites in `UIVirtualBoxManager.cpp` (33 `registerManagerText` + 7 `registerGlobalToolCommand`, counted with `grep -c '^\s*registerManagerText('` → 33 and `grep -c '^\s*registerGlobalToolCommand('` → 7), keys read but never registered, `%1` arity agreement, or translation quality — those stay T0. **The gate's blind spots are stated here so its existence is not read as broader coverage than it has.** (i) **Keys read but never registered are invisible to it.** It walks `registerText` call sites only; a key passed to `text()`/`md3Text()` that no `registerMd3*Texts()` function ever registers produces a silent runtime fallback and the gate reports `clean`. It can prove a registration is well-formed; it cannot prove a *reader* has one. (ii) **`%1`/`%2` placeholder-arity agreement between an English string and its Cantonese counterpart is not checked at all.** An English string with two placeholders paired with a Cantonese string carrying one — or zero — passes this gate, and the mismatch surfaces only at run time as a dropped `.arg()` value or a literal `%2` shown to the user. This is deliberate rather than overlooked: `md3.manager-tool.record-count` legitimately *reorders* its placeholders, so a naive equality check on arity would be wrong, and no arity check that tolerates reordering has been written. Since 2026-08-16 the gate does enforce one further class it previously only warned about — CI now passes `-StrictIdentical`, so an English string identical to its Cantonese counterpart fails the build instead of printing `WARN` and exiting 0, and the two hand-maintained exemptions are printed on every run as `identical-allow-listed: 2` with their reasons | B1 (compiles in `UICommon`) | None (C0) | — |
...
| S7 Documentation site | Partial | `docs/index.html` carries an inline three-mode switcher (`english`/`cantonese`/`bilingual`) mirroring `UIMd3LanguageMode`: same order, same `" · "` (U+00B7) bilingual join, same empty-Cantonese-falls-back-to-English rule, same English default, persisted in `localStorage` under `md3.language.mode` with the same `qBound(0,…,2)` clamp as `GUI/Md3/LanguageMode`. No network requests, no `innerHTML`, English is the literal DOM text so the page is complete without JavaScript. The playfulness sliders are **not** mirrored. | [`SiteLanguage.md`](SiteLanguage.md) | EN+ZH covers the page furniture, all 3 tab labels, the tablist accessible name, and **18 of the page's 22 headings** (`grep -oE '<h[1-3][ >]' docs/index.html \| wc -l` → 22; `grep -oE '<h[1-3] data-md3-zh' docs/index.html \| wc -l` → 18). The four untranslated headings are the `<h1>` product name `Material Virtual Machine` and the three Verification card `<h3>`s — `UICommon`, `VirtualBox` and `VirtualBoxVM` — which are build-target identifiers, not prose. Also translated: all 12 coverage card titles, the whole Overview panel, and the 2 Verification status phrases. **The body prose is only partly translated:** the 12 coverage card bodies, the 4 coverage footnotes and the Verification lede and closing paragraph — the bulk of the prose — stay English, and the page says so to the reader in Cantonese and bilingual modes. The 30 `doc/md3/*.md` files `pages.yml` publishes beside it are English-only raw Markdown the switcher cannot reach. **No percentage of translated characters is quoted here on purpose.** The previous version of this cell claimed "1,248 of 8,355 visible characters (14.9%)" and a byte ratio against `index.html`; neither reproduced, neither named a command, and both went stale the next time `docs/index.html` was touched. The two `grep`s above are printed because they do reproduce. | T1 — required literals, the U+00B7 join, negative guards and the `$pagesFacts` per-fact occurrence table in `md3-validation.yml`'s "Validate MD3 source wiring" step (read `$pagesFacts` for the current set; it grows whenever a fact is pinned); deliberate mutations were confirmed to make each class throw | 48 of 48 behavioural checks in a real headless Chrome over the DevTools Protocol (real key events, network capture, script-disabled reload) — **but that harness is a scratch file outside the repository and is not a repeatable gate**; see gate 20 in [`LocalGates.md`](LocalGates.md) | None (C0) — 3 headless screenshots were taken and read during verification, but they are of a static documentation page, not of the built application, so they are not capture evidence under `CaptureMatrix.md` | The long technical prose and all 30 published Markdown articles are untranslated by design in this pass; no claim is made about the deployed site, which was not fetched |
...
| S4/S6/S7 tabbed navigation | Partial (S7 only; S4/S6 not implemented) | S7 has a 3-tab ARIA tablist (Overview/Coverage/Verification) in `docs/index.html` with working arrow-key/Home/End navigation and `role=tablist`/`role=tab`/`role=tabpanel`; S4 wizards and S6 runtime have no tab strip of any kind | [`docs/index.html`](../../docs/index.html), [`SiteLanguage.md`](SiteLanguage.md) | EN+ZH for the three tab labels and the tablist's `aria-label`, through the site language switcher | T0 | N/A (static HTML, manually reviewed) | None (C0) | S7's 3 tabs have none of the required overflow, reorder, pin, group, four-discovery-search, or docking behavior — it is a minimal, real, but far from complete tab strip |
...
| S7 | Partial | Basic keyboard tab navigation and `focus-visible` styling in `docs/index.html`, plus a `role="radiogroup"` language switcher with roving `tabindex`, arrow/Home/End selection and a `role="status"` live region; all buttons measured at 48 px; an explicit `@media(prefers-reduced-motion:reduce)` block; no accessible-name audit performed | [`SiteLanguage.md`](SiteLanguage.md) | EN+ZH chrome — tab labels, tablist `aria-label`, headings, page furniture; the long prose stays English | T0 | Behaviour driven in a real headless browser (48 checks, gate 20 in [`LocalGates.md`](LocalGates.md)), but with a harness that is not checked in; **no screen-reader or accessibility-tree audit of any kind was performed** | None (C0) | — |
...
**S7 is Partial, not Implemented, and must not be rounded up:** only part of the page gets
Cantonese — 18 of its 22 headings do, the `<h1>` and the three Verification card `<h3>`s do not, and
the body prose is only partly translated — the 30 `doc/md3/*.md` articles published beside it are
English-only and out of the switcher's reach, and the behavioural harness that exercised it is a
scratch file outside the repository rather than a checked-in gate. The heading fraction is the only
number here because it is the only one that reproduces from a command
(`grep -oE '<h[1-3][ >]' docs/index.html | wc -l` → 22,
`grep -oE '<h[1-3] data-md3-zh' docs/index.html | wc -l` → 18). A character percentage and a byte
ratio used to stand here; both were wrong and both are deleted rather than re-derived, because a
figure that changes every time `docs/index.html` is edited cannot be kept true by care.
