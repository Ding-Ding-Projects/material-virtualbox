...
- [`Changelog.md`](Changelog.md) documents the in-app changelog viewer: every `CHANGELOG.md` entry compiled in and verified against real commits, the modeless <kbd>Ctrl+Shift+L</kbd>/command-palette browser with version/category/date filters, regex search, clickable per-entry commit references, and filtered Markdown export/copy.
