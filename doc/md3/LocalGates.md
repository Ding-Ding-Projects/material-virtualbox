...
that re-run. **Row 2 committed the exact defect this paragraph warns about and was repaired on
2026-08-16:** it recorded a line range and a line count for its CI step taken from a tree it had not
been run against, and later commits on this branch moved both. Its cell now carries no line range and
no line count at all — the step is named, not located — and a fresh run against the tree this
document is committed in. Rows 3–8 and 20 were **not** re-run this pass and still describe the tree
they were run against; row 7's inputs are known to have moved and its cell says so.
...
2. Every step that is pure source/text validation (pattern matching against
   ...
   **Defect in this recipe, found and reproduced 2026-08-16 — read this before
   trusting any exit code recorded in this document.** The recipe as written
   above does **not** reproduce GitHub Actions' exit-code semantics, and a lane
   following it can record **PASS on a gate that actually failed**. GitHub does
   not run a `pwsh` step body as-is: it wraps it, prepending
   `$ErrorActionPreference = 'stop'` and **appending**

   ```powershell
   if ((Test-Path -LiteralPath variable:\LASTEXITCODE)) { exit $LASTEXITCODE }
   ```

   That epilogue is what turns an inner script's `exit 1` into a failed job.
   Without it, a `.ps1` invoked from inside another `.ps1` sets `$LASTEXITCODE`
   in the caller but terminates only itself — the wrapper then runs off its own
   end and exits **0**. Reproduced directly rather than reasoned about, with a
   two-line `failing-gate.ps1` whose whole body is
   `Write-Host '  FAIL  simulated gate violation'` then `exit 1`:

   | Wrapper invoked as `pwsh -NoProfile -File …` | stdout | exit |
   | --- | --- | ---: |
   | `./failing-gate.ps1` (the recipe above) | `  FAIL  simulated gate violation` | **0** |
   | `./failing-gate.ps1` + the epilogue (what CI runs) | `  FAIL  simulated gate violation` | **1** |

   The two runs print byte-identical output and disagree only in the exit code,
   which is precisely why this is silent.

   **The epilogue is therefore a required part of the extraction recipe**, not
   an optional refinement: every extracted step must be wrapped as
   `$ErrorActionPreference = 'stop'`, the step body, then the `LASTEXITCODE`
   line, and only then does a local exit code mean what CI would mean by it.

   **And stdout must be read, not only the exit code.** The gate scripts here
   print `FAIL …` lines and a `violations: N` tally before they exit, so the
   text says what happened even when the status does not. This is not
   hypothetical either: while verifying gate 19 for this pass, a run piped
   through `| tail -8` reported `EXITCODE=0` while its own captured stdout read
   `FAIL  IDENTICAL EN/ZH …` and `violations: 1` — the pipeline had replaced the
   script's exit code with `tail`'s. Re-run without the pipe, the same tree
   returned exit **1**. Treat a bare `0` as evidence of nothing until the
   accompanying `RESULT: clean` line has been read.
...
| Material 3 validation | [31851996370](https://github.com/Ding-Ding-Projects/material-virtualbox/actions/runs/31851996370) (on `main`), [31856303192](https://github.com/Ding-Ding-Projects/material-virtualbox/actions/runs/31856303192) (on the `v7.2.97-ci.108` tag ref), [31851966893](https://github.com/Ding-Ding-Projects/material-virtualbox/actions/runs/31851966893) (on `codex/native-windows-ci-build-20260809`) | **FAILURE** |

`--commit` matches by `headSha` across every ref, so all three of those runs are the same tree on
three different refs. Only the first is a run *on `main`*, which matters for the streak below.

**`Material 3 validation` is red on `main` and has been since `8762579`, not since `fe321a4`.**
An earlier version of this section named `fe321a4` — the tip — as the start of the streak. It is the
*end* of it. The gate has failed on `main` at three consecutive commits, an hour apart:

`gh run list --repo Ding-Ding-Projects/material-virtualbox --workflow "Material 3 validation" --branch main --limit 15`

| Commit on `main` | Run | Started | Conclusion |
| --- | --- | --- | --- |
| `8762579681594cf8ba6beb8ccec77576baa5199a` | [31848342250](https://github.com/Ding-Ding-Projects/material-virtualbox/actions/runs/31848342250) | `2026-08-14T22:53:24Z` | **failure** |
| `753602ced18e50649ce0b0a3038cf7efd1ba97fe` | [31851883297](https://github.com/Ding-Ding-Projects/material-virtualbox/actions/runs/31851883297) | `2026-08-14T23:53:35Z` | **failure** |
| `fe321a4fd6f6bfc7c2fbd7e0704e3f72d0eb2eee` | [31851996370](https://github.com/Ding-Ding-Projects/material-virtualbox/actions/runs/31851996370) | `2026-08-14T23:55:34Z` | **failure** |

The last green run on `main` is
[31846486407](https://github.com/Ding-Ding-Projects/material-virtualbox/actions/runs/31846486407) at
`5f1adfac63246fcd97bf4a34f13813c563f60afe`. **`v7.2.97-ci.106` (target `8762579`) and
`v7.2.97-ci.107` (target `753602c`) therefore both shipped with this gate already red**, which the
understated "since `fe321a4`" hid: it made the red look like one commit and one release, when it is
three commits and three releases. The failing step is
...
| 2 | MD3 source-wiring contract (kBuild source lists, headers, wizard/palette/style/settings/manager/tab/regex/history/notification/theme source-text contracts, README + `docs/index.html` contracts) | Extracted verbatim from `md3-validation.yml`'s "Validate MD3 source wiring" step; run via `pwsh -NoProfile -File step2-md3-source-wiring.ps1` from the repo root | ~35 production `.cpp`/`.h` files under `src/VBox/Frontends/VirtualBox/src/md3/`, `Makefile.kmk`, `README.md`, `docs/index.html` | **RUN** | **PASS** | **Re-extracted and re-run after the `docs/index.html` language-switcher contract was added to this step**, because the earlier recorded run predates that change and no longer describes the step that exists. The whole body of the `Validate MD3 source wiring` step was de-indented into a scratch `step2-md3-source-wiring.ps1` outside the repository and invoked from the repository root under PowerShell 7.6.5. stdout: `step2 PASSED`; exit 0; runtime 0.62s. **No line range or line count is recorded for this step, deliberately.** Earlier versions of this cell pinned "lines 30–286" and "257 lines of YAML"; both were true of the tree they were extracted from and false at HEAD within two commits, which is the quiet inflation this document's own header forbids. The step is identified by name — it is the only step called `Validate MD3 source wiring` in `.github/workflows/md3-validation.yml` — and a reader who wants its bounds should read the file, not this cell. The earlier "~200 individual string/regex contracts" estimate is left uncorrected only in the sense that it was never counted precisely; the language-switcher block adds 13 required literals, 1 middle-dot literal, 3 negative guards, and 9 occurrence-count assertions on top of it. Seven deliberate mutations of `docs/index.html` (removing `role="radiogroup"`, renaming the mode list, changing the bilingual join to `/`, changing the persistence key, pasting a build hash into a Cantonese attribute, giving a switcher button `role="tab"`, and switching a node builder to `innerHTML`) were each confirmed to make this gate throw, and the file was restored and the gate re-run to `step2 PASSED` / exit 0 afterwards. **Independently re-run 2026-08-16 at `992aa84304a`**, because this is the step that is red in CI on `main` and a second pair of hands should confirm the repair rather than inherit it: the step body was de-indented verbatim into a scratch `step2.ps1` outside the repository, invoked from the repository root under PowerShell 7.6.5 — stdout `step2 PASSED`, exit 0, 0.48s. **Re-run again 2026-08-16 against the tree this row is committed in**, because later commits on this branch changed this very step — so the run above was evidence about a step that no longer exists, exactly the borrowed-evidence defect this row used to commit. Every future edit to the step owes this row a fresh run; `git log --oneline -- .github/workflows/md3-validation.yml` says whether one is owed. The step body was re-extracted at that tree, `if ((Test-Path -LiteralPath variable:\LASTEXITCODE)) { exit $LASTEXITCODE }` appended, and run from the repository root under PowerShell 7.6.5: **no stdout, exit 0** (the `step2 PASSED` echo above came from the earlier scratch wrapper, not from the step, and is not re-added). Both new negative guards were proved able to fail rather than assumed to be: injecting `url(` into a scratch-modified `docs/index.html` threw `The documentation site must stay network-free`, exit 1, and injecting an `@font-face` block threw the same, exit 1; the file was restored with `git checkout -- docs/index.html` and the gate re-run to exit 0 both times. **This is still a local run against a tree that carries `ca97ec93ccc`. CI has never executed this version of the step; at `fe321a4`, which lacks the repair, CI's copy of this same step throws `Compact README contract missing: no verified installer is published yet` (runs 31851996370 on `main`, 31856303192 on the tag ref).** **Re-run once more 2026-08-16, at the tree that adds the unsigned-driver ceiling contract** — the step changed again, so this row owed another run. Re-extracted verbatim, same `LASTEXITCODE` epilogue, run from the repository root under PowerShell 7.6.5: **no stdout, exit 0.** All four new assertions were proved able to fail rather than assumed to be, each against a scratch-mutated working tree that was restored immediately afterwards: deleting the `<p id="ceiling">` paragraph from `docs/index.html` threw `Pages unsigned-driver ceiling contract missing: id="ceiling"`, exit 1; pasting a second `VBoxSup.sys` into an unrelated Cantonese attribute threw `Technical fact must appear exactly 1 time(s) so language modes cannot diverge: VBoxSup.sys (found 2)`, exit 1; removing the `%2` slot from the ceiling's `data-md3-zh` string threw `The ceiling statement must render in all three language modes with the driver name slotted, not duplicated`, exit 1; and deleting the ceiling blockquote from `README.md` threw `README unsigned-driver ceiling contract missing: no virtual machine can start`, exit 1. The restored tree returns exit 0. |
...
| 7 | Release line-count and attribution table | `pwsh -NoProfile -File tools/count-release-lines.ps1` | Every `git ls-files`-tracked file, as enumerated by the script **at `cb9f573030e`** | **RUN** at `cb9f573030e`; **NOT re-run** since | **PASS** (self-checks all satisfied; exit 0, not the documented failure exit code 2) | Project total 26,272 files / 9,955,529 lines / 9,070,265 non-blank (vendored excluded); grand total 66,320 files / 25,865,478 lines. Agent-written (surviving): 23,721 lines / 21,713 non-blank, from 192 discovered agent commits blamed across 161 candidate files (123 blamed, 38 binary). Runtime 135s. Full table in this audit's run log. **Input drift noted 2026-08-16:** every figure in this cell was measured at `cb9f573030e` and the tree has changed since. The 135-second script was **not** re-run this pass, so no corrected totals are offered — the PASS verdict is not disproven, it is simply older than the tree it describes, and anyone quoting these numbers should re-run the script first. (An earlier version of this cell subtracted a `git ls-files \| wc -l` count from the script's own grand total to state a file delta. The two are not the same population — `git ls-files` also lists the kBuild gitlink, which the script's blob total does not — so the delta was arithmetic across two different definitions and has been deleted rather than corrected.) |
...
| 19 | MD3 language registry completeness (localization gate) | `pwsh -NoProfile -File tools/md3/check-language-registry.ps1 -MaxUnparseable 2 -StrictIdentical` | 427 compile-time `registerText` registrations resolving to 407 distinct keys across 22 sources under `src/VBox/Frontends/VirtualBox/` | **RUN** | **PASS** — but only after the defect it found on its own first run was fixed | Added after this document's header audit, so **this row was verified against a later tree than `cb9f573…`, not against that commit.** The first run exited 1: `FAIL  CONFLICTING KEY  'md3.wizard.current-page-description' registered 2x with 2 different texts:`, naming `src/VBox/Frontends/VirtualBox/src/md3/UIMd3Wizard.cpp:51 EN='Title of the active wizard page.'` and `src/VBox/Frontends/VirtualBox/src/wizards/UINativeWizard.cpp:91 EN='Current wizard page: %1'`. That is a real accessibility defect, not a cosmetic clash: `registerText` ends in `QHash::insert`, which overwrites, so whichever `registerMd3*Texts()` free function runs last wins and the other reader is always wrong — either `UINativeWizard.cpp`'s `.arg(pPage->title())` finds no `%1` and silently drops the page title from the page stack's accessible description, or `UIMd3Wizard.cpp`'s title label announces a literal `%1`. Fixed in commit [`ca97ec93ccc06559d1035dd039ef56ca7b000a93`](https://github.com/Ding-Ding-Projects/material-virtualbox/commit/ca97ec93ccc06559d1035dd039ef56ca7b000a93) by renaming the native wizard's key to `md3.wizard.page-stack-description` at both its registration and its one reader; the shared MD3 wizard shell keeps the generic name. The re-run printed `parsed registrations : 427`, `distinct keys        : 407`, `files scanned        : 22`, `violations: 0   warnings: 0`, `RESULT: clean`, exit 0, runtime ~1.9s. It also found 0 empty keys, 0 empty English strings and 0 empty Cantonese strings; 20 keys are registered more than once with *identical* text (deliberate co-registration by two owners) and pass by design; the only two identical English/Cantonese pairs — `md3.application` (product name) and `md3.regex.flags-placeholder` (PCRE2 flag letters `i m s x`) — sit in the script's hand-maintained allow-list with a written reason each, and any other identical pair warns (or fails under `-StrictIdentical`). **Standing blind spot the gate prints on every run, including clean ones:** 2 call sites in `src/VBox/Frontends/VirtualBox/src/manager/UIVirtualBoxManager.cpp` (lines 2711 and 2834) take key/English/Cantonese as runtime lambda parameters; the 40 registrations behind them (33 `registerManagerText`, 7 `registerGlobalToolCommand`) are counted as `unparseable, NOT CHECKED` and are **not** verified by this gate. CI passes `-MaxUnparseable 2` to freeze that number so a third invisible call shape fails the build instead of disappearing. **Independently re-run 2026-08-16 at `992aa84304a`** under PowerShell 7.6.5. Verbatim stdout, complete:<br>`== UIMd3Language registry completeness ==`<br>`  parsed registrations : 427`<br>`  distinct keys        : 407`<br>`  files scanned        : 22`<br>`  unparseable, NOT CHECKED : 2`<br>`    ? src/VBox/Frontends/VirtualBox/src/manager/UIVirtualBoxManager.cpp:2711  non-literal argument`<br>`        registerText(QString::fromLatin1(pszKey), strEnglish, strCantonese)`<br>`    ? src/VBox/Frontends/VirtualBox/src/manager/UIVirtualBoxManager.cpp:2834  non-literal argument`<br>`        registerText(QString::fromLatin1(pszKey), strFallback, strCantonese)`<br>`  violations: 0   warnings: 0`<br>`  RESULT: clean`<br>exit 0. Note that the gate prints its own blind spot on a clean run — the two unparseable sites are in the passing output above, not hidden behind a failure. **Updated 2026-08-16 — the identical-EN/ZH check is now wired to fail.** `md3-validation.yml` previously invoked this script *without* `-StrictIdentical`, so an English string identical to its Cantonese counterpart printed `WARN` and the step still exited 0 — meaning the one defect class that most directly says "this string was never translated" could not fail the build. CI now passes `-StrictIdentical`. The tree reports `warnings: 0`, so this went green on arrival. Re-run at this commit through a wrapper carrying GitHub's own `if ((Test-Path -LiteralPath variable:\LASTEXITCODE)) { exit $LASTEXITCODE }` epilogue (see the recipe defect under "How this was built"), verbatim stdout, complete:<br>`== UIMd3Language registry completeness ==`<br>`  parsed registrations : 427`<br>`  distinct keys        : 407`<br>`  files scanned        : 22`<br>`  unparseable, NOT CHECKED : 2`<br>`    ? src/VBox/Frontends/VirtualBox/src/manager/UIVirtualBoxManager.cpp:2711  non-literal argument`<br>`        registerText(QString::fromLatin1(pszKey), strEnglish, strCantonese)`<br>`    ? src/VBox/Frontends/VirtualBox/src/manager/UIVirtualBoxManager.cpp:2834  non-literal argument`<br>`        registerText(QString::fromLatin1(pszKey), strFallback, strCantonese)`<br>`  identical-allow-listed: 2`<br>`    = 'md3.application' = 'Material Virtual Machine'  src/VBox/Frontends/VirtualBox/src/md3/UIMd3ManagerHeader.cpp:107  -- Product name; not translated.`<br>`    = 'md3.regex.flags-placeholder' = 'i m s x'  src/VBox/Frontends/VirtualBox/src/md3/UIMd3RegexBuilder.cpp:120  -- PCRE2 flag letters i m s x; not translatable.`<br>`  violations: 0   warnings: 0`<br>`  RESULT: clean`<br>exit 0. The `identical-allow-listed: 2` line is new: the allow-list used to be silent, so an exemption could grow without appearing in any CI log. **`-StrictIdentical` was proved able to fail, not assumed to be:** deleting the `md3.regex.flags-placeholder` reason from the allow-list made the same command print `FAIL  IDENTICAL EN/ZH  'md3.regex.flags-placeholder' = 'i m s x'  src/VBox/Frontends/VirtualBox/src/md3/UIMd3RegexBuilder.cpp:120` and `violations: 1   warnings: 0`, exit **1**; the entry was restored and the gate re-run to `RESULT: clean`, exit 0. Standing limit unchanged: nothing here was compiled. |
| 20 | Documentation-site language switcher, exercised in a real browser | A scratch Node 26 script outside the repository driving Chrome `--headless=new` over the DevTools Protocol against `file:///…/docs/index.html`: real `Input.dispatchKeyEvent` key presses, `Network.enable` request capture, `Emulation.setScriptExecutionDisabled`, and `Page.captureScreenshot` | `docs/index.html` only | **RUN** | **PASS** — 48 of 48 checks, exit 0 | Verified behaviour, not source text: English default with zero CJK in the page copy; the switcher revealed only by script; `aria-checked`/roving `tabindex` correct in all three modes; ArrowRight/ArrowLeft/ArrowUp/ArrowDown/Home/End all select and wrap; the existing Overview/Coverage/Verification tab strip still answers ArrowRight and End with the right panel shown; `document.title`, the tablist accessible name and the `role="status"` live region all follow the mode; `localStorage` holds `0`/`1`/`2` under `md3.language.mode`, a reload restores the stored mode, `banana` falls back to English and `9` clamps to bilingual; every visible technical fact (3 build hashes, both commit SHAs, both run numbers, `MSVC 14.44`, `Windows SDK 10.0.26100.0`, `Qt 6.8.3`, `#6750A4`, the build timestamp) renders exactly once in English and once in Cantonese, and in bilingual mode **five** distinct facts are cloned, not two. **Corrected 2026-08-16:** this clause read "only the two facts that sit inside a translated paragraph are cloned", which undercounted. Cloning happens whenever a `%N` placeholder in a `data-md3-zh` attribute pulls a child element into the Cantonese run (`cantoneseNodes()` → `slotNode()` in `docs/index.html`), so any fact inside such a slot appears in both the English run and the Cantonese run. Re-measured by reimplementing `render()`'s exact `englishNodes`/`cantoneseNodes`/`slotNode` logic over a parse of `docs/index.html` and counting each fact in visible text plus `href` values: `#6750A4` 1→2, `UICommon` 2→3, `CodexHandoff.md` 1→2, `ArchiveManifest.sha256` 2→4, `DesignCoverage.md` 2→4 — five facts, every one of them rising. The 48-of-48 verdict is not disturbed by this: the harness asserted per-fact occurrence counts that were correct, and it was this document's prose summary of them that was wrong; the 23 distinct `href` values are byte-identical in all three modes; exactly one network request is made across a load and two mode changes, and it is the page itself; with script execution disabled the English lede and the whole Overview panel are still in the document and the switcher stays `hidden`. Screenshots of all three modes at 1100×900 were captured and read. **Updated 2026-08-16 — this 48-of-48 PASS missed two real accessibility defects, which is the honest headline of this row.** An independent verifier found both in the same file the harness had just declared green: (a) the `role="status"` line was **blank on a fresh load** — the IIFE called `render(storedMode())` but never the status renderer, so a reader reloading in Cantonese got a fully Cantonese page, a correctly checked Cantonese radio, and an empty status line until they touched the control; (b) the three switcher labels' Chinese halves carried **no `lang` attribute**, violating WCAG 2.2 SC 3.1.2 Language of Parts — they are the only CJK visible in *all* three modes, including English mode where the page otherwise has zero `zh-HK` elements, so an English synthetic voice mispronounced the very control a Cantonese-seeking reader is hunting for. The harness asserted that the live region *follows the mode* and never asserted its content on an untouched load; it counted CJK characters and never counted *language-tagged* elements. Both were fixed and re-measured with the harness rebuilt to serve `docs/` over `http://127.0.0.1` (not `file://`, so `localStorage` has a real origin) and to record a document-start `MutationObserver` log. Fresh-load `[lang="zh-HK"]` element counts, before → after: English **0 → 3**, Cantonese **41 → 45**, bilingual **41 → 45**; fresh-load status text, before → after: `""` → `"Language mode: English"` / `"語言模式：粵語"` / `"Language mode: Bilingual · 語言模式：雙語"`. The verifier's reported `42` for the old page reproduces as the *post-click* count (`on load 41` / `after click 42`): the difference is the status line's own Cantonese run, which only existed once the control had been touched. The load-time text is rendered **without** firing an announcement by removing the live-region `role` before the first write and restoring it in a `requestAnimationFrame`; the observer recorded `role attribute: "status" -> null`, `status text written: … while role=null`, `role attribute: null -> "status"` in all three modes. Gate 2 now pins both repairs — the three literals that fix the render-then-restore-the-role ordering, the structural shape of all three switcher labels, and an exact count of three literal `lang="zh-HK"` occurrences — so *these two* cannot regress silently even though the browser harness still cannot. **This sentence was unsupported when it was written and is now supported.** Gate 2's evidence cell above recorded only a run at `992aa84304a`, which is *before* the commit that added those assertions, so the row was asserting a property of a workflow no recorded run had exercised. Gate 2 has since been re-extracted and re-run at the current branch tip — the first tree that actually contains these assertions — and passed, exit 0; see that cell. The two assertions were each proved able to fail when they were added: reverting the label spans produced `Language switcher label must tag its Chinese half lang=zh-HK: english`, exit 1, and deleting `statusLine.removeAttribute('role')` produced `Pages status line must render without announcing on load: statusLine.removeAttribute('role')`, exit 1. **This harness is a one-off local verification and is not checked into the repository, so it is not a repeatable gate** — nothing in CI or in `tools/` reproduces it, and a future regression in switcher *behaviour* (as opposed to the source literals gate 2 pins) would not be caught automatically. |
...
- **No installer functional/COM-registration test.** The Windows workflow verifies only that the produced assets are *unsigned* (`Get-AuthenticodeSignature` → `NotSigned`) and that the NSIS `VirtualBox-<version>-Setup.exe` exists. **Corrected 2026-08-16:** this bullet used to say "Squirrel assets" and to name `RELEASES` and a full `.nupkg`; neither is produced by the NSIS path, and Squirrel.Windows was retired on 2026-08-14. `grep -n -i "RELEASES\|nupkg" .github/workflows/windows-package-release.yml` is not silent — it returns six lines, and every one of them is the *word* "releases" in an unrelated role: a `gh release list` call and the `$releases` variable it fills (the previous-tag scan), a `gh api "repos/$env:GITHUB_REPOSITORY/releases?per_page=100"` call and its failure message (the dim-sum dish-reuse scan), a `winflexbison` download URL and a dim-sum photo asset URL, both under `github.com/.../releases/download/`. Every hit is this repository's or a third party's *GitHub Releases*; none is a Squirrel update feed (the literal `RELEASES` manifest file), and `grep -c -i nupkg` on the same file returns `0`. The gap itself is unchanged: nothing verifies the installer actually installs correctly, registers `VBoxSDS` and the COM classes it needs, or that an installed copy launches successfully. **And no installer test could ever lift the ceiling above it: `VBoxSup.sys` ships unsigned, 64-bit Windows refuses to load an unsigned kernel driver, code signing is permanently prohibited for this project, and therefore no virtual machine can start.** That is a policy consequence with an owner outside this codebase, not a missing gate.
...
- **Localization completeness — was "No localization-completeness gate for the new MD3 text registry", now gate 19.**
  ...
  **What it still does not cover, and what therefore remains genuinely ungated:** (a) keys that are
  ...
  house voice; (e) the 40 registrations behind `UIVirtualBoxManager.cpp:2711` and `:2834`, which it
