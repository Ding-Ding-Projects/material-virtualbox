...
- **No runtime capture / screenshot harness exists at all.** `doc/md3/CaptureMatrix.md` enumerates 45 required surface/state captures and every single row reads "Not captured," blocked by (A) no verified installer yet and (B) `REGDB_E_CLASSNOTREG` — the built `VirtualBox.exe` cannot even reach its manager window in this environment because no `VBoxSDS` COM service is registered. There is no automated tool anywhere in the repository that takes a real screenshot of the built application; every claim of "it renders" currently rests entirely on source-pattern text matching (gates 2–6 above), which `doc/md3/CodexHandoff.md` itself calls a "regression guard" that "cannot prove a button is visible, a key sequence works, or an accessibility tree is correct."
...
- **No localization-completeness gate for the new MD3 text registry.** `UIMd3Language` (`src/VBox/Frontends/VirtualBox/src/md3/UIMd3Language.h`) registers English/Cantonese string pairs (`registerText(key, english, cantonese)`) at dozens of call sites across the MD3 sources. Nothing verifies every registered key actually has both a non-empty English and a non-empty Cantonese string, or that no key is registered twice with conflicting text. The classic Qt `.ts`-file `lupdate`/`lrelease` pipeline (gate 14) would not catch this even if it were run, because the MD3 rewrite does not use `.ts` files for its own strings.
