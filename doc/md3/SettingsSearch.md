...
Global Preferences also provides a collapsed, persisted customization disclosure. Expanding it reveals the language mode (English, Hong Kong Cantonese, or bilingual) and two independent 1–5 funny-level sliders in an explicit label/control layout. Machine Settings deliberately omits this global-only panel. The controls call the shared `UIMd3Language` instance directly, emit live change notifications, and restore their values through the existing extra-data lifecycle rather than a second preferences store.
...
- A blank plain-text query restores single-page selector navigation and the normal expert/basic visibility rules. An active query deliberately switches to the existing cross-page filtering view; a valid regex reveals pages whose descriptions or descendants match it.
...
Suggested articles: [`SettingsShell.md`](SettingsShell.md), [`AppearanceSettings.md`](AppearanceSettings.md), [`DesignCoverage.md`](DesignCoverage.md), and the repository [build instructions](https://www.virtualbox.org/wiki/Build_instructions).
