...
void UIMd3Changelog::prepareEntries()
{
    ...
    addEntry(m_entries, strUnreleased, pipelineDate, QStringLiteral("Known issue"), strPipelineSection,
             QStringLiteral("Packaging step now fails later, on something new"),
             QStringLiteral(
                 "As of this push to main, the Material 3 validation workflow is green, but "
                 "the Windows package and release workflow is still red. It now gets "
                 "substantially further than any run before it -- past Qt, past the toolset "
                 "selection, past the redistributable path, through the full build and into "
                 "the packaging step's own self-check -- before failing because "
                 ...
                 "rather than reporting a structure-layout mismatch. This is a different "
                 "failure signature from anything fixed above and has not yet been "
                 "root-caused. No release has been published, and no verified Windows "
                 "installer exists as of this entry."),
             QStringLiteral("9efa7f522c69f3ce1b99940d13e96d534ee2a0c3"));

    addEntry(m_entries, strUnreleased, pipelineDate, QStringLiteral("Documentation"), QString(),
    ...
    addEntry(m_entries, strUnreleased, pipelineDate, QStringLiteral("Related work"), QString(),
             QStringLiteral("Material 3 runtime chrome branch reconciled with main (not yet merged)"),
             QStringLiteral(
                 ...
                 "before the pipeline fixes in this list. It lives on branch "
                 "claude/full-ui-rewrite-reconciled-20260813, is not an ancestor of main "
                 "(verified with git merge-base --is-ancestor), and main has since advanced "
                 "six further commits that branch has not absorbed. It is not shipped; it is "
                 "recorded so the next integration pass knows it exists and is not current."),
             QStringLiteral("1e59aca274c171b85a5e293e6fe8b70a04ccce27"),
             false /* fOnDefaultBranch */);
}
