...
static const UIMd3ViewingConditions &md3ViewingConditions()
{
    ...
    if (!s_fPrepared)
    {
        ...
        const double dAdaptingLuminance = (200.0 / M_PI) * md3YFromLstar(50.0) / 100.0;
...
static UIMd3Cam16 md3Cam16FromRgb(int iRed, int iGreen, int iBlue)
{
    ...
    double dHueDegrees = std::atan2(dB, dA) * 180.0 / M_PI;
    if (dHueDegrees < 0.0)
        dHueDegrees += 360.0;
    ...
    const double dEHue = 0.25 * (std::cos(dHuePrime * M_PI / 180.0 + 2.0) + 3.8);
...
{
    const UIMd3ViewingConditions &vc = md3ViewingConditions();
    const double dHueRadians = dHueDegrees * M_PI / 180.0;
